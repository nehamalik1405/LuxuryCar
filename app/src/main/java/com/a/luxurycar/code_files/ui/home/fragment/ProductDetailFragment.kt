...
class ProductDetailFragment :
    BaseFragment<ProductDetailViewModel, FragmentProductDetailBinding, ProductDetailRepository>(),
    OnMapReadyCallback {
    ...
    override fun getRepository() =
        ProductDetailRepository(ApiAdapter.buildApi(ApiService::class.java))
    ...
    private fun manageClickListener() {
        ...
        binding.btnEmail.setOnClickListener {
            ...
            binding.btnEmail.setBackgroundResource(R.drawable.drawable_button_background)
            ...
            binding.btnCall.setBackgroundResource(R.drawable.drawable_background_email_button)
        ...
        binding.btnCall.setOnClickListener {
            ...
            binding.btnCall.setBackgroundResource(R.drawable.drawable_button_background)
            ...
            binding.btnEmail.setBackgroundResource(R.drawable.drawable_background_email_button)
        ...
        binding.imgViewShare.setOnClickListener {
            val intent = Intent(Intent.ACTION_SEND)
            intent.setType("text/plain")
            intent.putExtra(android.content.Intent.EXTRA_TEXT, "Hey, download this app!");
            startActivity(Intent.createChooser(intent, "choose one"));
        }

        binding.imgViewHeart.setOnClickListener {
            if (SessionManager.isUserLoggedIn()) {
                callLikeAndDisLikeApi()
            } else {
    ...
    private fun callLikeAndDisLikeApi() {
        if (SessionManager.getUserRole().equals(Const.KEY_BUYER)) {
            likeAndDisLikeApi()
            viewModel.getWishListResponse.observe(viewLifecycleOwner, Observer {
                ...
                when (it) {
                    is Resource.Success -> {
                        if (it != null && it.values.status == 1) {
                            ...
                            if (it.values.message!!.equals("Car removed from wishlist")) {
                                binding.imgViewHeart.setImageResource(R.mipmap.ic_heart)
                            } else {
                                binding.imgViewHeart.setImageResource(R.mipmap.ic_like)
                            }
                        } else {
                            Toast.makeText(requireContext(), it.values.message, Toast.LENGTH_SHORT)
                                .show()
    ...
    private fun likeAndDisLikeApi() {
        val jsonObject = JSONObject()
        try {
            jsonObject.put(Const.PARAM_CAR_ID, "4")
        ...
        if (NetworkHelper.isNetworkAvaialble(requireContext())) {
            viewModel.getWishListResponse(body)
        } else {
                ...
                is Resource.Success -> {
                    if (it != null && it.values.status == 1) {
                        ...
                        if (it.values.data.carAds != null) {
                            ...
                            binding.txtViewLoremIpsumisSimplyDummyTextofPrinting.setText(Html.fromHtml(
                                it.values.data.carAds.get(0).description).toString())
                            binding.txtViewLocationAddress.text =
                                it.values.data.carAds.get(0).locationUrl
                            carId = it.values.data.carAds.get(0).id.toString()
                            binding.rootProductDetailPage.visibility = View.VISIBLE
                        }
                        setViewPager()
                        setDeatilOfCar()
    ...
    private fun setViewPager() {
        ...
        productDetailViewPagerAdapter =
            ProductDetailViewPagerAdapter(requireContext(), arrBannerCarImageList)
